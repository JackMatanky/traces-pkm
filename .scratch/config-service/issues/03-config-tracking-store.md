...
- **`ConfigFileStore::new` accepts only `paths::StateDirRoot`**, a newtype whose constructor is private to `paths.rs`. The only two values that can ever exist are `paths::TRACKED_CONFIGS` and `paths::TRUSTED_CONFIGS` — a production caller cannot construct a store pointed at an arbitrary or typo'd directory (a raw `&str`/`PathBuf` parameter was considered and rejected during review specifically because a typo'd directory name would compile silently and `clean()` would then delete files in the wrong place).
