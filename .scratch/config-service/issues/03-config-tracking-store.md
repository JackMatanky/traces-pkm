...
- **Store root is a preset constant at the production call site, not an injected field.** `paths::TRACKED_CONFIGS`/`TRUSTED_CONFIGS` are `LazyLock<PathBuf>` statics (matching mise's `dirs.rs` `TRACKED_CONFIGS`/`TRUSTED_CONFIGS` shape exactly). `ConfigTracker` passes `paths::TRACKED_CONFIGS` directly to `ConfigFileStore` — the root is not threaded through `ConfigService` as a field or constructor parameter. Test isolation is handled by passing explicit temp dirs to `ConfigFileStore`; tests that exercise `ConfigBuilder::track()` still avoid the real state dir because `paths::state_root()` redirects to the OS temp dir under `#[cfg(test)]`.
