...
- `docs/adr/0002-config-trust-and-tracking.md` — amended to de-emphasize templates as the trust *subject* (workspace root + config file are what's trusted; templates are the motivating risk and a downstream consumer, not independently trusted) and removed the stale "template-directory trust... still owed to issue 05" language that had confused this issue's first triage pass.
