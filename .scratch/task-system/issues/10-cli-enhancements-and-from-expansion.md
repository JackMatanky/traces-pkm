...
**Current behavior:**
`render_task_list` in `src/query/format.rs` converts all tasks to `"- [x] "` or
`"- [ ] "`, stripping custom status markers (`[/]`, `[-]`, `[!]`, `[?]`). All
tasks render at column 0 without indentation regardless of list depth. Suffixes
only include `({path})` without line numbers. `traces task` lacks convenience
filter shortcuts (`--todo`, `--done`, `--status`), sorting flags, count-only
output, and table view defaults. `--from` does not parse direct file paths.

**Desired behavior:**
`render_task_list` preserves the exact `TaskStatusSymbol` character and indents
items by `list.depth * 2` spaces. `--line-numbers` (`-l`) renders clickable
`({path}:{line})` suffix coordinates. Convenience flags (`--todo`, `--done`,
`--status <char>`) allow rapid task filtering. Flattened `SortArgs` enable
`--sort`, `--asc`, and `--desc` across any `list.*` or `file.*` path. `--table`
defaults to five key task columns (`Task`, `Status`, `Due`, `Priority`,
`File`). `--count` outputs matching count only. `SourceSelector::parse` accepts
direct `.md` file paths.

**Key interfaces:**

- `render_task_list`: Format using `row.task_status_symbol()` and indent by
  depth.
- `Task` CLI struct in `src/cli/task.rs`: Add `--line-numbers`, `--todo`,
  `--done`, `--status`, `--count`, `--table`, and flatten `SortArgs`.
- `SourceSelector::parse`: Add direct file path detection.
...
- Machine-readable `--json` and `--plain` flags.
