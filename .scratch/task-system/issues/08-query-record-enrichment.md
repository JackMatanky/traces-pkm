...
Expanding a task row eagerly clones strings (`text`, `clean_text`,
`status_name`), allocating tens of thousands of heap objects during query
expansion. `FieldPath::Task(TaskField)` parses `task.<field>` and rejects
generic list items, while generic list items have no representation in
`QueryRow`.
