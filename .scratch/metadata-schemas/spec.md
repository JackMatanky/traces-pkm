...
- Ticket 07 (`issues/07-values-file-source.md`) extends this spec: a Select/Multi Field Definition's `values` key becomes polymorphic — a literal array (unchanged) or a subtable pointing at an external TOML/JSON file (`path`, plus optional `value`/`label`) — motivated by the reference vault's `country`/`city`/`job_title`/`industry` fields (Metadata Menu's `ValuesListNotePath`). Not covered by this document's original Out of Scope list because it wasn't yet identified when this spec was written.
