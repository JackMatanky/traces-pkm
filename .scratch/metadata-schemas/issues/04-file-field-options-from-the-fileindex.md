# 04 — File-Field Options from the WorkspaceIndex

**What to build:** A `file`-typed Field Definition in a Schema resolves its option list live from the WorkspaceIndex: an AND-composed filter of `folders` (array), `ext`, and `class` (array). `.field()` on such a field returns label/value pairs — label from the `[frontmatter]` aliases key, else the configured title key, else the filename stem; value the path — so `ui.select` shows a friendly label and returns the path (per ADR-0003). The `class` filter matches transitively through extends. Option lists are index-derived at use-time, so only as fresh as the index. No regex in filters.

**Blocked by:** 03 — Schema minijinja Namespace (done on `feat/schema-namespace`, branch from there)

**Status:** implemented

- [x] `file` fields resolve their options from the WorkspaceIndex through an AND-composed filter of `folders` (array), `ext`, and `class` (array); no regex in filters.
...
- `WorkspaceIndex::file_options(...)` now returns deterministic label/value file options from the current index.
- `SchemaBinding::field()` now resolves `FieldType::File` through the render-scoped WorkspaceIndex cache and returns `[{ label, value }, ...]` objects.
...
- Reused `template::engine::query::cached_refresh()` so `schema`, `query`, and `tasks` share one render-scoped WorkspaceIndex refresh.
...
1. AND-composed `folders`/`ext`/`class` filter, no regex — `WorkspaceIndex::file_options` (`src/index/mod.rs`): folder any-of via `starts_with`, `ext` exact match after `trim_start_matches('.')`, class exact-set membership; no `regex` crate use anywhere in the path.
...
- One pre-existing (not introduced this session) terminology nit: `WorkspaceIndex::file_options`'s doc says "Empty `folders`, `ext`, or `classes` values skip that filter dimension" — `ext` is `Option<&str>`, not a collection, so calling `None` "empty" slightly conflates the two; could read "empty `folders`/`classes`, or an absent `ext`."
...
**Phase 6 — docs.** `spec.md`'s Testing Decisions now mention the title-fallback step in label precedence (was still "aliases... else stem" only). Tightened `WorkspaceIndex::file_options`'s doc: "empty `folders`/`classes`, or an absent `ext`" instead of lumping `ext: Option<&str>` in with "empty" collection language.
