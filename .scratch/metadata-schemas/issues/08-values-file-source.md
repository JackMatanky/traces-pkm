...
2. a subtable pointing at an external TOML or JSON file (`values = { path =
   ...
   **full resolved object** (every key the entry declared, not a narrowed
   ...
   `ui.select`'s index-based recovery (`get_item_by_index`, ADR-0003) selects
   among the returned items unchanged, so a template or a future MCP-facing
   consumer can read any declared key. File format is chosen by extension; both
...
Downstream, `ui.select` recovers the chosen item by index (`get_item_by_index`,
`src/template/engine/ui.rs`; ADR-0003 — still `proposed`, worth accepting
...
safe against `read_raw_schemas` (`src/schema/service.rs:200`): it reads only
`*.toml` files at `min_depth(1).max_depth(1)` — a `values/` subdirectory, in any
format, is never misread as a Schema/File Class.
...
- New `SchemaFieldParserError` variants for: bad extension,
  missing/unreadable/unparseable file, missing root `entries`, selector
