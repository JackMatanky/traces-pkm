...
**Blocked by:** 02 — Schema Registry and Field Resolution (implemented); 03 — Schema minijinja Namespace (implemented — this ticket's `.field()` return-shape change lands in the `Select` arm `SchemaBinding::field()` already wires up); 07 — Schema Domain Refactor: SchemaService and Deep Module Boundaries (not started — lands the `SchemaSelectFieldEntry`/`SchemaFieldBuilder`/`Schema*FieldDef` seam this ticket's polymorphic `values` sources build on).
...
- [ ] Amend ADR-0006's Consequences to name the load-time-external-but-static phase (`schema::SchemaFieldBuilder`, from ticket 07) as a first-class option alongside "declared in the TOML" and "index-derived at use-time" — deferred here rather than to ticket 07 because this is the first ticket that actually exercises that phase.
