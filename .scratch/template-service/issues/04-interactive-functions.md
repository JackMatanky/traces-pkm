...
- [x] `ui.text_input`, `ui.select`, `ui.confirm`, `ui.multi_select` callable from templates and delegate to the interactive provider
- [x] `ui.text_input` supports the optional default argument
- [x] With `PresetDialogProvider`, rendering is deterministic (no TTY required)
- [x] Tests render templates exercising each method and assert the output
...
- **AC amendment** (`.worktrees/interactive-functions`, same branch, on top of `6fed24b`): at the user's direction, amended the one criterion that didn't match what was actually built rather than reverting the design to match it. Changed:
  - Agent Brief's AC line ("Dry-run mode selects a defaults-only provider... resolves issue 03's known gap") now reads "A `--no-input` flag selects a defaults-only provider..., independent of `--dry-run`/TTY state" — describes the mechanism actually shipped instead of the one originally sketched at triage and later rejected mid-implementation (see "Implementation" comment above for why).
  - "Desired behavior" and "Key interfaces" (Agent Brief section) updated to match: no longer say `TemplateService`/dry-run selects the provider — `src/cli/template.rs`'s `--no-input` flag does, independent of `WriteMode`.
  - "Known gap (dry-run)" replaced with a "Resolved (dry-run)" note pointing at the design-discussion comments, since it no longer describes an open question.
  - Both checklists (top-level, line 24-27; Agent Brief, line 165-170) are now checked off in full — every item was independently re-verified against current source/tests in the "Acceptance-criteria audit" comment above before amending, so checking them here records a finding already made, not a new claim.
...
**Desired behavior:**
A `ui` namespace object (implementing `minijinja::value::Object`) is registered via `env.add_global("ui", ...)`, holding an `Arc<dyn DialogProvider>`. The four methods are callable from templates, each delegating to the corresponding `DialogProvider` method. `ui.text_input` supports an optional default argument (`Option<String>`). `ui.select` and `ui.multi_select` accept `Value` items (iterable, with label/value key detection). An independent `--no-input` CLI flag (not `--dry-run` itself) selects a defaults-only `PresetDialogProvider` so defaults are returned without TTY involvement, regardless of `--dry-run`/TTY state; `--dry-run` alone still previews with real answers when run interactively, since forcing empty defaults on every dry-run would make previews useless for templates whose output branches on a `ui.*` answer.

**Key interfaces:**
- `src/template/engine.rs` — register `ui` namespace; `TemplateEngine` needs `Arc<dyn DialogProvider>` at construction
- `src/cli/template.rs` — wire provider selection: `--no-input` → defaults-only provider, else the one from CLI setup; independent of `--dry-run`
...
**Acceptance criteria:**
- [x] `ui.text_input`, `ui.select`, `ui.confirm`, `ui.multi_select` callable from templates and delegate to `DialogProvider`
- [x] `ui.text_input` supports optional default argument
- [x] `ui.select`/`ui.multi_select` accept `Value` items, detect `label`/`value` keyed items, fall back to `to_string()`
- [x] A `--no-input` flag selects a defaults-only provider (not `TerminalDialogProvider`), independent of `--dry-run`/TTY state — resolves issue 03's known gap without forcing every dry-run preview to go blind
- [x] With `PresetDialogProvider`, rendering is deterministic (no TTY required)
- [x] Tests render templates exercising each method and assert output

**Resolved (dry-run):** the open question above was settled during implementation, not left as originally worded — provider selection ended up independent of `WriteMode` entirely; a dedicated `--no-input` flag covers the "no hang, no TTY required" need instead of `--dry-run` forcing it. See `## Comments`'s "Implementation" and "Acceptance-criteria audit" entries for the full design discussion and verification.
