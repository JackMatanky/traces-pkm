...
- `WriteOutcome` (`Written(PathBuf)` / `Previewed(String)`) and `TemplateWriter::write` — the one entry point on `TemplateWriter` — both live in `writer.rs`, not `service.rs`. First draft put both in `service.rs` (reasoning: `WriteOutcome` synthesizes engine content + writer target path, neither owned by `writer.rs` alone) and had `render_to_file` do the `DryRun`-vs-`choose`+`commit` branch itself. Corrected on review: `TemplateWriter::write(output, rendered: RenderOutput, mode, default) -> Result<WriteOutcome, TemplateError>` now owns that whole decision — for `DryRun` it returns `rendered.content` as `Previewed` without picking a target at all (so `-o`/`file.write_to()` is never confined, matching the "never compute an output path in dry-run" requirement below); otherwise it calls `choose` then `commit` (both now private, since nothing outside `write` calls them) and returns `Written`. `render_to_file` shrank to resolve + read + render + one delegating call: `self.writer.write(output, rendered, mode, || self.default_output_path(&resolved))`. `write` takes `rendered: RenderOutput` (the engine's own `{content, write_to}` product) rather than two separate params — bundling was needed to stay under `clippy.toml`'s `too-many-arguments-threshold = 5` (6 params including `self` otherwise), and `RenderOutput` already existed as exactly that shape, so `writer.rs` now imports it from `engine.rs` (a new, narrow, `pub(super)`-scoped sibling dependency; not a `writer::mod.rs`-documented invariant this violates).
