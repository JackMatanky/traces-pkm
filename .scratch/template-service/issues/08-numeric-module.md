...
`issue/08-numeric-module`, commit `2a5f438` (parent `0d9326a`). Merged
into `main` via a `--no-ff` merge (see commit history for the merge
commit); the one real conflict was in `engine.rs`'s doc comment
(issue 09's `StrOps` filter list vs. this issue's `NumOps` addition
to the same paragraph) and was resolved by combining both.
