...
- **File layout:** `src/template/num_ops.rs` — `NumOps` unit struct with `register(&self, env)` calling `env.add_filter(...)`.
...
- `.scratch/template-service/issues/05-includes-and-utility-functions.md` (establishes the `register` pattern)
