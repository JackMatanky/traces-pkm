...
**Architectural cost of timing**: Adopting an `Arc<WorkspaceIndex>` behind a swap-on-refresh point now **does not foreclose or complicate** adding `salsa` later. It is a highly compatible foundation. 
...
- By building the `Arc<WorkspaceIndex>` Host/Snapshot boundary now, Traces establishes the rigorous concurrency and cancellation model required for a responsive LSP. If performance targets later demand `salsa`, the internal `Arc<WorkspaceIndex>` can be cleanly swapped for a `salsa::Database` without rewriting the LSP request routing or thread-pool logic.
...
**Recommendation: Implement the `Arc<WorkspaceIndex>` snapshot model now; explicitly defer `salsa` to Ticket 33.**
...
3. **Extensible Foundation**: The Host/Snapshot pattern built around `Arc<WorkspaceIndex>` perfectly isolates the mutable state. It proves out the threading and cancellation architecture, serving as a ready-made drop-in shell if `salsa` proves necessary later.
