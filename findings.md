...
## Remaining ADR-0004 Scope — 2026-07-27
- Committed cutover `0fd7957` covers command outcomes and Template Resolution, but `src/cli/error.rs` still owns four command wrapper enums and nine erased `Box<dyn StdError>` source fields.
- The remaining deepening opportunity is one CLI presentation module whose interface accepts typed domain failures plus operation context, with stable code/help policy owned once.
- Config discovery/build currently has command-specific presentation identities in Template and completions; these must converge before the diagnostic contract is stable.
- `TemplateError::Render` retains minijinja's source but has no Custom Function classification. The next design pass must preserve minijinja location/detail while classifying actionable dialog, I/O, and Custom Function origins.
- No test currently drives the process-level Escape/Ctrl-C exit mapping or asserts that every interactive command leaves no side effect after User Abort.

### Refactor Reconnaissance — 2026-07-27
- Current planning remains aligned with source: ADR-0004 has completed the first cutover, but the four-command CLI presentation family, configuration source erasure, and render-origin classification remain.
- GitNexus confirms the remaining Template pipeline seams are `TemplateService` resolution/read/render propagation and `TemplateEngine::render`; it also identifies the CLI error, dialog, command dispatch, template command, completions, init, and integration-test modules as the relevant second-pass surface.
- The remaining refactor must not infer minijinja rendering causes from display text. It needs an explicit typed classification that preserves the original minijinja error as the source so location/detail remain intact.
- The physical centralization in `src/cli/error.rs` is not a deep module: `CliError` only wraps four shallow per-command error enums, while each repeats code/help behavior. `TemplateCliError` additionally pattern-matches domain errors to mint presentation policy, proving the policy is stranded on the wrong side of the CLI seam.
- `Template` and `Completions` duplicate a three-way current-directory/config-load translation. Both falsely classify a `Cwd::new()` failure as config discovery and erase the concrete source. A shared CLI configuration-loading module is a real seam: it has two adapters and would remove duplicated policy while preserving context.
- `Init` has no typed domain error module; it boxes dialog, TOML serialization, and I/O at its command adapter. It also has a real transactional concern: directory creation may succeed before later steps fail, leaving partial state. This must be explicitly classified rather than hidden behind one generic wrapper.
- Domain error classifications are mostly precise and source-preserving: `ConfigLoadError` distinguishes discovery/build; `ConfigFileError` retains parse/trust stages; `TemplateError` distinguishes resolve, read, output policy, render, write, and prompt; `DialogError` retains cancellation/interruption as exact variants. The remaining typed exceptions are deliberate external-erasure points (`figment`, `inquire`) or error-type recursion breaks, not CLI presentation decisions.
- `TemplateEngine` cannot distinguish syntax, `ui.*`, `file.include`, regex, date, or any other Custom Function failure from a bare `minijinja::Error` at its interface. The render module needs an explicit classification interface immediately around `TemplateEngine::render`, based on retained source chains and `ErrorKind`, before any CLI presentation refactor can safely assign stable codes/help.
- First clean-cutover slice complete: Template and completions now share `config_load::load`, which returns `ConfigCliError::{CurrentDirectory, Load}` with concrete `io::Error`/`ConfigLoadError` sources. This is the intended deep module for shared CLI configuration loading; completions-specific Config diagnostics and Template's duplicate Config variants are deleted.
- Trust target resolution no longer boxes either of its real causes. Reading `cwd` is `io::Error`; target discovery is `DiscoveryError`. These retain independent remediation while the public trust-command behavior remains unchanged.
