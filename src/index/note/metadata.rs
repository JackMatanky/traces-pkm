...
impl From<&RawFrontmatter> for Frontmatter {
    fn from(raw: &RawFrontmatter) -> Self {
        ...
        let fields = parse_yaml_frontmatter(raw.as_str());
        Self::new(fields)
    }
}
...
fn parse_yaml_frontmatter(raw: &str) -> Vec<MetadataField> {
    let val = match serde_yaml::from_str::<serde_yaml::Value>(raw) {
        Ok(v) => v,
        Err(err) => {
    ...
    let serde_yaml::Value::Mapping(map) = val else {
        warn!(
            "YAML frontmatter is not a key-value mapping; ignoring top-level \
             value"
        );
        return Vec::new();
    };
