...
use crate::{
    ...
    index::IndexError,
    query::{QueryBuilderError, QueryDialect, QueryError},
    schema::SchemaError,
...
const fn static_help(error: &CliError) -> Option<&'static str> {
    match error {
...
fn query_help(source: &QueryError) -> Box<dyn Display + 'static> {
    let help = match source {
        QueryError::Request(QueryBuilderError::Syntax(error))
        ...
        QueryError::Request(QueryBuilderError::FieldPath(_))
        | QueryError::FieldPath(_) => "check every referenced field path",
        QueryError::Request(QueryBuilderError::LimitOutOfRange {
            ..
