...
//!
//! The loader is hand-rolled, not `minijinja::path_loader`:
...
/// Renders minijinja template sources, with `{% include %}`/`{% extends %}`
/// resolving against the local then global template directory.
pub(super) struct TemplateEngine {
    env: Environment<'static>,
}

impl TemplateEngine {
    /// Creates an engine whose loader searches `local_dir` then
    /// `global_dir` — mirroring [`super::resolve::resolve_template`]'s
    /// directory priority — for `{% include %}`/`{% extends %}`.
    #[inline]
    #[must_use]
    pub(super) fn new(
        ...
        env.set_loader(build_loader(local_dir, global_dir));
        Self {
            env,
        }
    }
    ...
    /// load or itself fails to render.
    #[inline]
...
fn build_loader(
    ...
    move |name| {
        let Ok(template_path) =
            TemplatePath::try_from(std::path::Path::new(name))
        else {
            return Ok(None);
        };
        for dir in
        ...
        {
            match fs::read_to_string(dir.join(&template_path)) {
...
mod tests {
    ...
    fn render_resolves_include_from_local_dir() {
        ...
        let engine = TemplateEngine::new(Some(temp.path().to_path_buf()), None);
    ...
    fn render_resolves_a_dot_prefixed_base_directory() {
        ...
        let engine = TemplateEngine::new(Some(dir), None);
    ...
    fn render_falls_back_to_global_when_missing_from_local() {
        ...
        fs::write(global_dir.join("shared.md"), "from global")
            .expect("write template");
        let engine = TemplateEngine::new(Some(local_dir), Some(global_dir));
