...
//!   design, so [`path`] and [`loader`] both depend on it directly
//!   instead of on each other.
//! - [`path`]: [`TemplatePath<State>`](path::TemplatePath), a name's
//!   journey from raw `-i` argument to a file proven to exist —
//!   [`Raw`](path::Raw) -> [`Validated`](path::Validated) ->
//!   [`Found`](path::Found) — with
//!   [`TemplatePathError`](path::TemplatePathError) as the single error
//!   type for every way that journey can fail.
//! - [`loader`]: [`TemplateLoader`](loader::TemplateLoader), the
//!   directory search — one method, [`loader::TemplateLoader::find`],
//!   used everywhere so local-before-global precedence never drifts.
//!   Never escapes the configured directories: an absolute path or a
//!   `..` segment is always a miss, never a traversal.
//! - [`engine`]: wraps minijinja's
//!   [`Environment`](minijinja::Environment) so [`service`] depends on
//!   "render this source" rather than on minijinja's API.
//! - [`service`]: [`TemplateService`], which chains resolve, render, and
//!   write into the one call `crate::cli::template` makes.
//!
