...
mod tests {
    ...
    mod merge_fields {
        ...
        fn first_listed_parent_wins_a_shared_field() {
            ...
            raw.insert(SchemaName::for_test("child"), schema(&["a", "b"], &[]));
        ...
        fn emits_warning_when_parent_failed_to_resolve() {
            ...
            assert!(warnings.contains(&SchemaWarning::ParentFailedToResolve {
                ...
                parent: SchemaName::for_test("broken"),
            }));
    ...
    mod reference {
        ...
        fn to_nonexistent_ancestor_field_degrades_to_failure() {
            let mut raw = IndexMap::new();
            raw.insert(
                SchemaName::for_test("ancestor"),
                schema(&[], &[("status", select_field(&["draft"]))]),
            );
            raw.insert(
                SchemaName::for_test("descendant"),
                schema(&["ancestor"], &[(
                    "ghost",
    ...
    mod hierarchy {
        ...
        fn is_a_matches_transitively_through_extends() {
            let mut raw = IndexMap::new();
            raw.insert(SchemaName::for_test("thing"), schema(&[], &[]));
            raw.insert(SchemaName::for_test("book"), schema(&["thing"], &[]));
        ...
        fn failed_link_breaks_hierarchy_downstream() {
            ...
            assert!(
                !book.descendants().contains(&SchemaName::for_test("sci_fi")),
                "book.descendants() must not list sci_fi: sci_fi no longer \
                 is-a book"
            );
            assert!(
                !book.descendants().contains(&SchemaName::for_test("broken"))
            );
        }
    }
    ...
    mod resilience {
        ...
        fn missing_extends_target_degrades_with_a_warning() {
            ...
            assert!(warnings.contains(&SchemaWarning::MissingExtendsTarget {
                schema: SchemaName::for_test("sci_fi"),
                target: SchemaName::for_test("ghost"),
            }));
            assert!(warnings.contains(&SchemaWarning::ParentFailedToResolve {
                schema: SchemaName::for_test("sci_fi"),
                parent: SchemaName::for_test("ghost"),
            }));
            let sci_fi =
        ...
        fn malformed_field_does_not_block_unrelated_sibling() {
            ...
            assert_eq!(failure.schema, SchemaName::for_test("broken"));
        }
        ...
        fn multiple_schemas_fail_independently() {
            ...
            assert!(failed_schemas.contains(&&SchemaName::for_test("alpha")));
            assert!(failed_schemas.contains(&&SchemaName::for_test("beta")));
        }
    }
}
