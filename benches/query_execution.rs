...
//! ### Isolation Rationale
//!
//! [`bench_execute_pages_by_metadata`] measures filter and sort combined, which
//! cannot attribute a regression to either component or to field resolution.
//! [`bench_filter_by_metadata_field_count`] and [`bench_sort_by_metadata`]
//! isolate each half; the former sweeps frontmatter field count per note to
//! distinguish O(1) hash-keyed lookup from an O(K) linear scan.
//!
...
/// The size sweep (not a single point) exists so the result can be fit as `A·n
/// + B·n·log₂(n)`: the linear term isolates per-row key
...
fn bench_sort_by_metadata(c: &mut Criterion) {
    ...
    for n in [5_000_usize, 10_000, 20_000, 40_000] {
        let index = built_index(n);
        group.throughput(Throughput::Elements(
            u64::try_from(n).expect("note count fits u64"),
        ));
...
/// Deterministic LCG: lets permutation benchmarks shuffle identically on every
...
/// Fisher-Yates shuffles `items` in place using the shared deterministic LCG.
...
///
/// Stress-test decomposition of [`bench_sort_by_metadata`]'s 4.7 ms: if a full
