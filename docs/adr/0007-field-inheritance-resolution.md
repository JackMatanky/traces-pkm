...
- The Confirmation section's original "resolution is a pure function of the schema set" was stated unconditionally; ticket `07-schema-service-refactor` scoped it to the `extends`/`$ref`/Kahn's-sort linearization specifically (see Confirmation below) once `schema/fields.rs`'s `SchemaFieldBuilder` made the field-construction seam explicit
...
Resolution is a pure function of the schema set for the `extends`/`$ref`/Kahn's-sort linearization specifically (`schema/graph.rs`'s DAG walk and `schema/fields.rs`'s `RefResolver`) — Kahn's sort and `$ref` resolution are unit-testable with no vault. This is narrower than the original unqualified claim: a `select` field's declared `values` and a `file` field's `folders`/`ext`/`class` filter are validated against the field's resolved type by `schema/fields.rs`'s `SchemaFieldBuilder`, itself pure, but a `file` field's *option list* still resolves live from the `FileIndex` outside `resolve()` entirely (per ADR-6), and ticket `07-schema-service-refactor` confirmed no third case needs threading through `resolve()`'s signature to stay pure. Tests assert: transitive is-a matching (a sci-fi note matches a book class query); first-listed-wins among parents; own-fields-override-parents; `excludes` dropping inherited fields; cycles hard-error; a missing extends target degrades to exact match with a warning; a `$ref` to global and to an ancestor resolves with local-key overrides; a stray `required = true` in `global.toml` is ignored with a warn log while a referencing schema's local `required` holds.
