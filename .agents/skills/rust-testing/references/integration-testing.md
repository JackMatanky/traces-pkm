...
| `cargo nextest run --lib` | `cargo nextest run --test '*'` |
...
A `tests/common/mod.rs` file is not itself compiled as a test binary (it has no `test_` file directly under `tests/` matching the harness pattern used by cargo — files in subdirectories aren't auto-discovered as separate test crates), so it's the right place for shared setup:
...
```bash
cargo nextest run --test '*'              # all integration tests
cargo nextest run --test integration_test # one file
cargo nextest run -E 'test(login)'        # tests matching a name filter
```

See [`running-tests.md`](running-tests.md) for the full command reference.

## See Also

- [`unit-testing.md`](unit-testing.md) — the complementary in-crate test layer
- [`doctests.md`](doctests.md) — the third test layer, focused on documented public-API examples
- [`running-tests.md`](running-tests.md) — commands and CI wiring
